...
class DataLoader:
    ...
    def load_data(self):
        if self.use_s3:
            ...
            df = self.spark_session.read.csv(f"s3a://{s3_path}", header=True, inferSchema=True)
        else:
    ...
    def load_police_station_data(self):
        if self.use_s3:
            s3_path = os.path.join(Config.BRONZE_S3_PATH, Config.LOCAL_FILENAME_STATIONS).replace("\\", "/")
            print("Loading police station data from S3...")
            df = self.spark_session.read.csv(f"s3a://{s3_path}", header=True, inferSchema=True)
        else:
            local_file_path = Config.get_data_path(Config.LOCAL_FILENAME_STATIONS)
